        ...
        ConfigContent?.map((config) => (
          <div key={config?.id} className="text-center flex flex-col justify-center items-center gap-2 sm:px-0">
            <div className="relative pb-[56.25%] overflow-hidden rounded-md w-full">
              <iframe className="absolute w-full h-full left-0 right-0 max-w-full border-none mx-auto" src={config.video} title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
            </div>
